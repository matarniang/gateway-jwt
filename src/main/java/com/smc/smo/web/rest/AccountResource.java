...
import com.smc.smo.repository.UserRepository;
import com.smc.smo.security.SecurityUtils;
import com.smc.smo.service.MailService;
import com.smc.smo.service.UserService;
import com.smc.smo.service.dto.AdminUserDTO;
import com.smc.smo.service.dto.PasswordChangeDTO;
import com.smc.smo.web.rest.errors.*;
import com.smc.smo.web.rest.vm.KeyAndPasswordVM;
import com.smc.smo.web.rest.vm.ManagedUserVM;
...
public class AccountResource {
    ...
    /**
     * {@code POST  /enable} : verife activate the registered user.
     */
    @PostMapping("/enable")
    public Mono<User> EnableUser(@RequestBody String request) {
    	JSONObject object=new JSONObject(request);
        String login=object.getString("login"); 
        log.debug("login ------------->" + login );
        return userService.VerifEnable(login);
    }
    
    /**
     * {@code POST  /verificationkey} : verife activate the registered user.
     */
    @PostMapping("/verificationKey")
    public Mono<User> verificationKey(@RequestBody String request){
    	JSONObject object=new JSONObject(request);
        String key=object.getString("key"); 
        log.debug(" key -------------> " + key);
        return userService.verificationKey(key);
    }
    ...
    public Mono<Void> requestPasswordReset(@RequestBody String request) {
        JSONObject object=new JSONObject(request);
